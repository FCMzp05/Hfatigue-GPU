#!/usr/bin/env python3
"""Cui et al. (2024) Fig. 8 — self-contained run + plot script.

Reproduces the pre-charging comparison of
...
Fig. 8 studies the influence of H2 pre-charging before mechanical loading
at R = 0.1 and f = 1 Hz for pH2 = 55 and 106 MPa.  Two model conditions
are compared per pressure:
  - "test condition" (solid lines): 24 h pre-charge, i.e. exactly the
    Fig. 6 runs, whose results.csv files are read from the fig6 runs_v22
    output directory; and
  - "fully charged" (dash-dot lines): the initial hydrogen concentration
    is set uniformly to the environmental concentration C_env everywhere
    before cycling starts.  These two cases are computed by this script.
...
two fully-charged load cases and the final plotting all live here.  Only
the CT mesh (generated once, stored under outputs/) is read from disk.

Physics summary (identical to the validated Fig. 6 script, except that
the 24 h pre-charge is replaced by a uniform C = C_env initial state):
  ...
  - Stress-assisted lattice diffusion with environmental concentration on
    the outer boundary and (penalty-enforced) on newly created crack
    faces; the skipped cycles of a block are homogenised with the mean
    min/max hydrostatic stress (cycle-averaged diffusion).

Usage:
    # run every case that has no finished result yet, then plot
    python cui2024_fig8_precharging_ct_q8.py --case all
    # run one case
    python cui2024_fig8_precharging_ct_q8.py --case p106_r01_f1_full
    # only regenerate the figure from existing runs
    python cui2024_fig8_precharging_ct_q8.py --plot-only
"""
...
FIG_DIR = ROOT / "outputs" / "cui_2024_hydrogen_fatigue" / "fig8"
FIG6_RUNS_DIR = (
    ROOT / "outputs" / "cui_2024_hydrogen_fatigue" / "fig6" / "runs_v22"
)
PAPER_MESH = (
...
VERSION = "cui2024_fig8_selfcontained_v1"

# --- The two "fully charged" Fig. 8 load cases: pressure (MPa), load
# ratio, frequency (Hz), load range (N).  Identical loading to the R=0.1,
# f=1 Hz Fig. 6 hydrogen cases; only the initial concentration differs. ---
CASES: dict[str, dict] = {
    "p55_r01_f1_full": {"pressure": 55.0, "ratio": 0.1, "frequency": 1.0, "delta_p": 7684.0},
    "p106_r01_f1_full": {"pressure": 106.0, "ratio": 0.1, "frequency": 1.0, "delta_p": 7684.0},
}
...
def run_case(name: str, args) -> None:
    ...
    if args.resume and state_path.exists():
        ...
        # Fig. 8 "fully charged" condition: instead of a finite 24 h
        # pre-charge, the lattice starts uniformly saturated at C_env.
        state["c"] = jnp.full(nnode, c_env, dtype=dtype)
        print(
            f"fully charged initial condition: C={c_env_wppm:.4f} wppm "
            f"uniform ({c_env:.6e} mol/mm^3)",
            flush=True,
        )
    ...
    print("=" * 78)
    print(f"Cui 2024 Fig. 8 case {name} — Q8 upper-half CT")
    print(
    ...
    summary = {
        "paper": "Cui et al., IJHE 72 (2024) 315-325, Fig. 8",
        "version": VERSION,
        "doi": "10.1016/j.ijhydene.2024.05.264",
        "case": name,
        ...
        "parameters": {
            ...
            "D_mm2_s": DIFFUSIVITY,
            "precharge": "fully charged (uniform C=Cenv initial condition)",
...
# =========================================================================
# Plotting: 24 h pre-charge (fig6 runs) vs fully charged (this script)
# =========================================================================
def growth_rate_curve(results_csv: Path, window_mm: float = 0.8):
    """da/dN (m/cycle) versus DeltaK from a results.csv history.

    A centred fixed-extension secant regression is used so accelerating
    trajectories retain their high-rate end without mesh-scale noise.
    """
    ...
    offsets = np.linspace(-half, half, 31)
    rates = []
    for center in centers:
        local_extension = center + offsets
...
def plot_figures(args) -> None:
    ...
    curves = (
        (
            args.fig6_runs_dir / "p55_r01_f1" / "results.csv",
            "#1348E3", "-", "$p_{H_2}$ = 55 MPa (24 h charge)",
        ),
        (
            args.fig6_runs_dir / "p106_r01_f1" / "results.csv",
            "#EE2724", "-", "$p_{H_2}$ = 106 MPa (24 h charge)",
        ),
        (
            args.runs_dir / "p55_r01_f1_full" / "results.csv",
            "#1348E3", "-.", "$p_{H_2}$ = 55 MPa (fully charged)",
        ),
        (
            args.runs_dir / "p106_r01_f1_full" / "results.csv",
            "#EE2724", "-.", "$p_{H_2}$ = 106 MPa (fully charged)",
        ),
    )
    ...
    paper_data = FIG_DIR / "experiments" / "paper_fig8_model_curves.csv"
    if paper_data.exists():
        digitized = np.genfromtxt(
            paper_data, delimiter=",", names=True, dtype=None, encoding="utf-8"
        )
        for condition, color, style in paper_curves:
            selected = digitized[digitized["condition"] == condition]
            if len(np.atleast_1d(selected)):
                axis.plot(
                    np.atleast_1d(selected["delta_K_mpa_sqrt_m"]),
                    np.atleast_1d(selected["da_dN_m_per_cycle"]),
                    style,
                    color=color,
                    lw=1.15,
                    marker="o",
    ...
    for results, color, style, label in curves:
        if not results.exists():
            print(f"missing results, skipping curve: {results}", flush=True)
            continue
        dk, rate = growth_rate_curve(results)
        if len(dk):
            axis.plot(
                dk, rate, style, color=color, lw=2.2, label=label, zorder=3
            )
    axis.set(
        xscale="log", yscale="log",
        xlim=(5.0, 20.0), ylim=(5.0e-9, 5.0e-7),
        xlabel=r"$\Delta K$ (MPa$\sqrt{\mathrm{m}}$)",
        ylabel=r"$da/dN$ (m/cycle)",
        title=(
            "Cui et al. (2024) Fig. 8 — influence of pre-charging "
            "(R = 0.1, f = 1 Hz)"
        ),
    )
    ...
    handles.extend(
        (
            Line2D([], [], color="0.25", lw=2.2),
            Line2D(
                [], [], color="0.25", lw=1.15, marker="o", ms=2.8,
                mfc="white", mew=0.65,
            ),
        )
    )
    labels.extend(("Current reproduction", "Cui et al. digitized model"))
    axis.legend(handles, labels, frameon=False, fontsize=8.7, loc="upper left")
    target = final_dir / "fig8_reproduction.png"
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        ...
        type=Path,
        default=(
            ROOT / "outputs" / "cui_2024_hydrogen_fatigue" / "fig6"
            / "runs_sensitivity" / "alpha0_10p5_prediction"
        ),
        help="matching 24 h Fig. 6 runs used as the test condition",
    )
    result.add_argument(
        "--alpha-0",
        type=float,
