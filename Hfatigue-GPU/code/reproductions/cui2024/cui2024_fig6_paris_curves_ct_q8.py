#!/usr/bin/env python3
"""Cui et al. (2024) Fig. 6 — self-contained run + plot script.

Reproduces the four da/dN-vs-DeltaK panels of
...
eight Fig. 6 load cases and the final plotting all live here.  Only the
...
Usage:
    ...
    python cui2024_fig6_paris_curves_ct_q8.py --case all
    # run one case
    python cui2024_fig6_paris_curves_ct_q8.py --case p106_r01_f1
    # only regenerate the figures from existing runs
    python cui2024_fig6_paris_curves_ct_q8.py --plot-only
"""
...
FIG_DIR = ROOT / "outputs" / "cui_2024_hydrogen_fatigue" / "fig6"
PAPER_MESH = (
    ROOT / "outputs" / "cui_2024_hydrogen_fatigue" / "fig5" / "mesh"
    / "cui_ct_half_q8_paper.msh"
)
EXPERIMENT_DIR = FIG_DIR / "experiments"
STUDY_PAPER = "Cui et al., IJHE 72 (2024) 315-325, Fig. 6"
STUDY_DOI = "10.1016/j.ijhydene.2024.05.264"
STUDY_LABEL = "Cui 2024 Fig. 6"
GEOMETRY_NOTE = (
    "B=25 mm follows geometric scaling of the nominal B=12.7, W=26 mm "
    "experiments to paper W=50 mm."
)
...
VERSION = "cui2024_fig6_selfcontained_v1"

# --- The eight Fig. 6 load cases: pressure (MPa), load ratio, frequency
# (Hz), load range (N).  DeltaP reconstructs the digitized initial DeltaK
# of every experimental curve. ---
CASES: dict[str, dict] = {
    "air_r01_f1": {"pressure": 0.0, "ratio": 0.1, "frequency": 1.0, "delta_p": 17289.0},
    ...
    "p55_r05_f3": {"pressure": 55.0, "ratio": 0.5, "frequency": 3.0, "delta_p": 5763.0},
    "p55_r05_f10": {"pressure": 55.0, "ratio": 0.5, "frequency": 10.0, "delta_p": 5763.0},
}
...
def make_solvers(nodes_np, elem_np, n_np, grad_np, b_np, dofs_np, det_np,
    ...
    def state(u, d):
        ...
        # Eq. (15) states alpha=psi. Keep the previously validated g*psi
        # interpretation available as an explicit diagnostic branch.
    ...
    def solve_c(
        ...
        step = jnp.asarray(elapsed_s, dtype=dtype)
...
def hydrogen_factor_from_wppm(wppm):
...
def hydrogen_factor(concentration_q):
    ...
    return hydrogen_factor_from_wppm(wppm)


def fatigue_factor(alpha_bar, alpha_0, exponent=2.0):
    return (alpha_0 / (alpha_bar + alpha_0)) ** exponent
...
def save_state(
    ...
    np.savez_compressed(
        ...
        adaptive_jump=np.asarray(adaptive_jump),
        accepted_block_count=np.asarray(accepted_block_count),
        rejected_block_count=np.asarray(rejected_block_count),
...
def run_case(name: str, args) -> None:
    ...
    if args.delta_p_override is not None:
        case["delta_p"] = args.delta_p_override
    ...
    def advance_concentration(cold, hydro, elapsed_s, damage):
        """Advance diffusion over explicit physical time with optional substeps."""
        if args.diffusion_time_integrator == "subcycle":
            substeps = max(1, math.ceil(elapsed_s / args.diffusion_max_step_s))
        else:
            substeps = 1
        substep_s = elapsed_s / substeps
        concentration = cold
        maximum_residual = jnp.asarray(0.0, dtype=dtype)
        for _ in range(substeps):
            ...
            maximum_residual = jnp.maximum(maximum_residual, residual)
        return concentration, maximum_residual, substeps

    def advance_hydrogen_memory(memory_start, concentration_q, elapsed_s):
        """Exact finite-relaxation update; tau=0 recovers the Cui law."""
        target_wppm = concentration_q * (1.0e6 * M_H / RHO_M)
        if args.hydrogen_memory_tau_s <= 0.0:
            return target_wppm
        survival = jnp.exp(
            -jnp.asarray(elapsed_s / args.hydrogen_memory_tau_s, dtype=dtype)
        )
    ...
    run_signature = json.dumps(
        {
            ...
            "fatigue_degradation_exponent": args.fatigue_degradation_exponent,
            "alpha_e": ALPHA_E,
            "mean_stress_kappa": MEAN_STRESS_KAPPA,
            "hydrogen_memory_tau_s": args.hydrogen_memory_tau_s,
            "fatigue_energy": args.fatigue_energy,
            "fatigue_cycle_driving": args.fatigue_cycle_driving,
            "hydrogen_scaled_alpha_n": args.hydrogen_scaled_alpha_n,
    ...
    if args.resume and state_path.exists():
        ...
        if "run_signature" in saved:
            saved_signature = str(saved["run_signature"])
            if saved_signature != run_signature:
                raise RuntimeError(
                    "checkpoint physics/calibration parameters do not match this run"
                )
        else:
            print(
                "WARNING: legacy checkpoint has no parameter signature; "
        ...
        if "hydrogen_memory" in saved:
            state["hydrogen_memory"] = jnp.asarray(
                saved["hydrogen_memory"], dtype=dtype
            )
        else:
            resumed_concentration_q = jnp.einsum(
                "qi,ei->eq",
                jnp.asarray(n, dtype=dtype),
                state["c"][jnp.asarray(elements)],
            )
            state["hydrogen_memory"] = (
                resumed_concentration_q * (1.0e6 * M_H / RHO_M)
            )
        resumed_adaptive_jump = int(
            saved["adaptive_jump"] if "adaptive_jump" in saved else args.cycle_jump
        )
        accepted_block_count = int(
            saved["accepted_block_count"] if "accepted_block_count" in saved else len(rows)
        )
        rejected_block_count = int(
            saved["rejected_block_count"] if "rejected_block_count" in saved else 0
        )
    else:
        cycle, rows = 0, []
        resumed_adaptive_jump = args.cycle_jump
        ...
        state = {
            ...
            "hydrogen_memory": jnp.zeros(
                (len(elements), len(n)), dtype=dtype
            ),
        }
        ...
        for step in range(args.precharge_steps):
            ...
            precharge_concentration_q = jnp.einsum(
                "qi,ei->eq",
                jnp.asarray(n, dtype=dtype),
                state["c"][jnp.asarray(elements)],
            )
            state["hydrogen_memory"] = advance_hydrogen_memory(
                state["hydrogen_memory"],
                precharge_concentration_q,
                precharge_dt,
            )
    ...
    print("=" * 78)
    print(f"{STUDY_LABEL} case {name} — Q8 upper-half CT")
    print(
    ...
    adaptive_jump = (
        args.cycle_jump
        if args.fixed_cycle_jump or args.reset_adaptive_jump_on_resume
        else resumed_adaptive_jump
    )
    stable_accepts = 0
    element_nodes = jnp.asarray(elements)
    shape_values = jnp.asarray(n, dtype=dtype)
    quadrature_measure = jnp.asarray(det, dtype=dtype)

    def hydrogen_inventory_mol_per_mm(block_state: dict):
        """Integrate lattice hydrogen over the modeled half-domain area."""
        concentration_q = jnp.einsum(
            "qi,ei->eq", shape_values, block_state["c"][element_nodes]
        )
        return jnp.sum(quadrature_measure * concentration_q)

    def advance_cycle_block(initial_state: dict, block_jump: int):
        """Advance one coupled block without mutating ``initial_state``."""
        ...
        concentration_at_minimum = initial_state["c"]
        hydrogen_memory_start = initial_state["hydrogen_memory"]

        # Minimum-load state.  Diffusion later uses the mean min/max
        # hydrostatic stress (or the split half-cycle operators).
        block_state["u"], ur_min = solve_u(
            d_start, zero_boundary, initial_state["u"], pmin
        )
        ...
        block_state["u"], ur = solve_u(
            d_start, zero_boundary, block_state["u"], pmax
        )
        stagger_error = np.inf
        dr = jnp.asarray(0.0, dtype=dtype)
        cr = jnp.asarray(0.0, dtype=dtype)
        hydrogen_q = jnp.ones_like(alpha_bar_start)
        fatigue_q = fatigue_factor(
            alpha_bar_start, args.alpha_0, args.fatigue_degradation_exponent
        )
        elapsed_block_s = block_jump / case["frequency"]
        diffusion_substeps = 1
        for stagger in range(1, args.max_stagger + 1):
            old_d = block_state["d"]
            old_c = block_state["c"]
            psi, alpha, _, hydro = qois(block_state["u"], block_state["d"])
            history_trial = jnp.maximum(history_start, psi)
            if args.diffusion_cycle_mode == "split":
                concentration_half, cr_min, substeps_min = advance_concentration(
                    concentration_at_minimum,
                    hydro_min,
                    0.5 * elapsed_block_s,
                    block_state["d"],
                )
                block_state["c"], cr_max, substeps_max = advance_concentration(
                    concentration_half,
                    hydro,
                    0.5 * elapsed_block_s,
                    block_state["d"],
                ...
                diffusion_substeps = substeps_min + substeps_max
            else:
                diffusion_hydro = 0.5 * (hydro_min + hydro)
                block_state["c"], cr, diffusion_substeps = advance_concentration(
                    concentration_at_minimum,
                    diffusion_hydro,
                    elapsed_block_s,
                    block_state["d"],
            ...
            block_state["hydrogen_memory"] = advance_hydrogen_memory(
                hydrogen_memory_start, concentration_q, elapsed_block_s
            )
            hydrogen_q = hydrogen_factor_from_wppm(
            ...
            alpha_n_scale = (
                ...
                else jnp.ones_like(hydrogen_q)
            )
            fatigue_alpha = (
                jnp.maximum(alpha - alpha_min, 0.0)
            ...
            trial, dr = solve_d(
                history_trial, fatigue_q, block_state["d"], hydrogen_q
            )
            block_state["d"] = jnp.clip(
                jnp.maximum(trial, d_start), 0.0, 1.0
            )
            block_state["u"], ur = solve_u(
                block_state["d"], zero_boundary, block_state["u"], pmax
            )
            damage_error = jnp.linalg.norm(
                block_state["d"] - old_d
            ) / jnp.maximum(jnp.linalg.norm(block_state["d"]), 1.0e-30)
            concentration_error = jnp.linalg.norm(
                block_state["c"] - old_c
            ) / jnp.maximum(jnp.linalg.norm(block_state["c"]), 1.0e-30)
            stagger_error = float(
        ...
        # Refresh peak-load concentration using the accepted damage field.
        _, _, _, hydro = qois(block_state["u"], block_state["d"])
        if args.diffusion_cycle_mode == "split":
            concentration_half, cr_min, substeps_min = advance_concentration(
                concentration_at_minimum,
                hydro_min,
        ...
        diagnostics = {
            ...
            "elapsed_block_s": elapsed_block_s,
            "diffusion_substeps": diffusion_substeps,
            "fatigue_q": fatigue_q,
            "hydrogen_q": hydrogen_q,
        }
        return block_state, diagnostics

    def combined_half_diagnostics(first: dict, second: dict):
        return {
            "residuals": [
                max(left, right)
                for left, right in zip(first["residuals"], second["residuals"])
            ],
            ...
            "elapsed_block_s": (
                first["elapsed_block_s"] + second["elapsed_block_s"]
            ),
            "diffusion_substeps": (
                first["diffusion_substeps"] + second["diffusion_substeps"]
            ),
    ...
    while cycle < args.max_cycles:
        ...
        initial_extension = crack_extension(
            nodes, np.asarray(jax.device_get(state_before_block["d"]))
        )
        full_state, full_diagnostics = advance_cycle_block(
            state_before_block, jump
        )
        trial_diagnostics = [full_diagnostics]
        block_trial_solves = 1
        block_error_ratio = 0.0
        block_crack_increment_error = 0.0
        block_crack_increment_error_mm = 0.0
        block_phase_error = 0.0
        block_concentration_error = 0.0
        block_hydrogen_memory_error = 0.0
        block_hydrogen_inventory_error = 0.0

        if args.block_error_control and jump >= 2:
            first_half_jump = jump // 2
            second_half_jump = jump - first_half_jump
            first_half_state, first_half_diagnostics = advance_cycle_block(
                state_before_block, first_half_jump
            )
            state, second_half_diagnostics = advance_cycle_block(
                first_half_state, second_half_jump
            )
            diagnostics = combined_half_diagnostics(
                first_half_diagnostics, second_half_diagnostics
            )
            trial_diagnostics.extend(
                (first_half_diagnostics, second_half_diagnostics)
            )
            block_trial_solves = 3

            full_extension = crack_extension(
                nodes, np.asarray(jax.device_get(full_state["d"]))
            )
            half_extension = crack_extension(
                nodes, np.asarray(jax.device_get(state["d"]))
            )
            full_increment = full_extension - initial_extension
            half_increment = half_extension - initial_extension
            block_crack_increment_error_mm = abs(
                full_increment - half_increment
            )
            block_crack_increment_error = (
                block_crack_increment_error_mm
                / max(abs(half_increment), args.block_crack_increment_atol_mm)
            )
            block_phase_error = float(
                jax.device_get(
                    jnp.linalg.norm(full_state["d"] - state["d"])
                    / jnp.maximum(jnp.linalg.norm(state["d"]), 1.0e-30)
                )
            )
            block_concentration_error = float(
                jax.device_get(
                    jnp.linalg.norm(full_state["c"] - state["c"])
                    / jnp.maximum(jnp.linalg.norm(state["c"]), 1.0e-30)
                )
            )
            block_hydrogen_memory_error = float(
                jax.device_get(
                    jnp.linalg.norm(
                    ...
                    / jnp.maximum(
                        jnp.linalg.norm(state["hydrogen_memory"]), 1.0e-30
                    )
                )
            )
            full_inventory = hydrogen_inventory_mol_per_mm(full_state)
            half_inventory = hydrogen_inventory_mol_per_mm(state)
            block_hydrogen_inventory_error = float(
                jax.device_get(
                    jnp.abs(full_inventory-half_inventory)
                    / jnp.maximum(jnp.abs(half_inventory), 1.0e-30)
                )
            )
            controlled_error_ratios = [
                block_crack_increment_error_mm
                / (
                    args.block_crack_increment_atol_mm
                    + args.block_crack_increment_rtol * abs(half_increment)
                ),
                block_phase_error / args.block_phase_rtol,
                block_concentration_error / args.block_concentration_rtol,
            ]
            if args.hydrogen_memory_tau_s > 0.0:
                controlled_error_ratios.append(
                    block_hydrogen_memory_error / args.block_memory_rtol
                )
            block_error_ratio = max(controlled_error_ratios)
        ...
        for trial in trial_diagnostics:
            ...
            if not all(
                ...
                raise RuntimeError(
                    f"linear residual limit exceeded: {trial['residuals']}"
                )

        maximum_damage_increment = float(
            jax.device_get(
                jnp.max(state["d"] - state_before_block["d"])
            )
        )
        stagger_error = diagnostics["stagger_error"]
        error_control_reject = (
        ...
        if (error_control_reject or convergence_reject) and jump > args.min_cycle_jump:
            ...
            print(
                ...
                f"coupled_err={stagger_error:.3e}; "
                f"block_err={block_error_ratio:.3e}; retry jump={adaptive_jump}",
                flush=True,
            )
            continue
        if error_control_reject:
            raise RuntimeError(
                "block error limit exceeded at minimum jump "
                f"{jump}: ratio={block_error_ratio:.3e} "
                f"(crack={block_crack_increment_error:.3e}, "
                f"phase={block_phase_error:.3e}, "
                f"concentration={block_concentration_error:.3e})"
            )
        if stagger_error >= args.stagger_tol:
            raise RuntimeError(
        ...
        if (
            ...
            and maximum_damage_increment < 0.25 * args.max_damage_increment
        ):
            stable_accepts += 1
            if stable_accepts >= 8:
        ...
        stagger = diagnostics["stagger"]
        elapsed_block_s = diagnostics["elapsed_block_s"]
        diffusion_substeps = diagnostics["diffusion_substeps"]
        fatigue_q = diagnostics["fatigue_q"]
        hydrogen_q = diagnostics["hydrogen_q"]
        damage_host = np.asarray(jax.device_get(state["d"]))
        concentration_host = np.asarray(jax.device_get(state["c"]))
        hydrogen_memory_host = np.asarray(
            jax.device_get(state["hydrogen_memory"])
        )
        inventory_mol_per_mm = float(
            jax.device_get(hydrogen_inventory_mol_per_mm(state))
        )
        inventory_mol_full_specimen = (
            2.0 * THICKNESS * inventory_mol_per_mm
        )
        ...
        row = {
            ...
            "maximum_hydrogen_memory_wppm": float(
                hydrogen_memory_host.max()
            ),
            ...
            "tip_zone_maximum_hydrogen_memory_wppm": float(
                hydrogen_memory_host[tip_zone].max()
            ),
            "hydrogen_inventory_mol_per_mm_half_domain": (
                inventory_mol_per_mm
            ),
            "hydrogen_inventory_mol_full_specimen": (
                inventory_mol_full_specimen
            ),
            "hydrogen_inventory_mg_full_specimen": (
                inventory_mol_full_specimen * M_H * 1.0e6
            ),
            ...
            "elapsed_block_s": elapsed_block_s,
            "diffusion_substeps": diffusion_substeps,
            "diffusion_substep_s": elapsed_block_s / diffusion_substeps,
            "maximum_damage_increment": maximum_damage_increment,
            "block_error_ratio": block_error_ratio,
            "block_crack_increment_error": block_crack_increment_error,
            "block_crack_increment_error_mm": block_crack_increment_error_mm,
            "block_phase_error": block_phase_error,
            "block_concentration_error": block_concentration_error,
            "block_hydrogen_memory_error": block_hydrogen_memory_error,
            "block_hydrogen_inventory_error": (
                block_hydrogen_inventory_error
            ),
            "block_trial_solves": block_trial_solves,
            "Ru_min": residuals[0],
            "Ru_max": residuals[1],
            "Rd": residuals[2],
            "Rc_max": residuals[3],
        ...
        if cycle <= 3 or blocks_this_run % args.print_every == 0:
            print(
                ...
                f"block_err={block_error_ratio:.2e}",
                flush=True,
            )
    ...
    summary = {
        ...
        "final_hydrogen_inventory_mol_full_specimen": rows[-1][
            "hydrogen_inventory_mol_full_specimen"
        ],
        "final_hydrogen_inventory_mg_full_specimen": rows[-1][
            "hydrogen_inventory_mg_full_specimen"
        ],
        "mesh": str(mesh_path),
        "nodes": nnode,
        ...
        "parameters": {
            ...
            "reset_adaptive_jump_on_resume": args.reset_adaptive_jump_on_resume,
            "block_error_control": args.block_error_control,
            "block_crack_increment_rtol": args.block_crack_increment_rtol,
            "block_crack_increment_atol_mm": (
                args.block_crack_increment_atol_mm
            ),
            "block_phase_rtol": args.block_phase_rtol,
            "block_concentration_rtol": args.block_concentration_rtol,
            "block_memory_rtol": args.block_memory_rtol,
            "hydrogen_memory_tau_s": args.hydrogen_memory_tau_s,
            "diffusion_cycle_mode": args.diffusion_cycle_mode,
            "diffusion_time_integrator": args.diffusion_time_integrator,
            "diffusion_max_step_s": args.diffusion_max_step_s,
            "fatigue_energy": args.fatigue_energy,
            "fatigue_cycle_driving": args.fatigue_cycle_driving,
            "hydrogen_scaled_alpha_n": args.hydrogen_scaled_alpha_n,
        ...
        "notes": [
            ...
            GEOMETRY_NOTE,
            "Fatigue and diffusion both advance over the accepted cycle block; "
            "the diffusion time is DeltaN/f and is integrated either as one "
            "implicit block step or bounded physical-time substeps.",
            "The load history seen by diffusion is cycle-averaged in average mode "
            "or represented by minimum/maximum half-cycle operators in split mode.",
            "The direct nonsymmetric concentration weak form is solved "
            "with BiCGSTAB.",
            "A positive hydrogen-memory relaxation time activates the exact "
            "finite-rate bulk degradation state; tau=0 is the Cui limit.",
            "Hydrogen inventory integrates lattice concentration over the "
            "modeled half-domain and multiplies by 2B to report the mirrored "
            "full-specimen value; it excludes trap occupancy.",
        ],
    }
...
def plot_figures(args) -> None:
    ...
    paper_path = EXPERIMENT_DIR / "paper_fig6_model_curves.csv"
    paper_curves = None
    if paper_path.exists():
        paper_curves = np.genfromtxt(
            paper_path, delimiter=",", names=True, dtype=None, encoding="utf-8"
        )
    ...
    for (panel, config), axis in zip(PANELS.items(), axes.ravel()):
        for case_name, tone, color, label in config["curves"]:
            ...
            summary = args.runs_dir / case_name / "summary.json"
            if (
                (not results.exists() or not summary.exists())
                and args.fallback_runs_dir is not None
            ):
                results = args.fallback_runs_dir / case_name / "results.csv"
                summary = args.fallback_runs_dir / case_name / "summary.json"
            if not results.exists() or not summary.exists():
                continue
            completed = json.loads(summary.read_text(encoding="utf-8")).get(
                "completed", False
            )
            if not completed:
                continue
            if paper_curves is not None:
                published = paper_curves[
                    (paper_curves["panel"] == panel)
                    & (paper_curves["case"] == case_name)
                ]
                if len(np.atleast_1d(published)):
                    axis.plot(
                        np.atleast_1d(published["delta_K_mpa_sqrt_m"]),
                        np.atleast_1d(published["da_dN_m_per_cycle"]),
                        "--",
                        color=color,
                        lw=1.25,
                        alpha=0.8,
            ...
            dk, rate = growth_rate_curve(
                results, window_mm=args.growth_rate_window_mm
            )
            if len(dk):
                axis.plot(
                    dk, rate, "-", color=color, lw=2.2, label=label, zorder=3
                )
        axis.set(
    ...
    fig.suptitle(
        "Cui et al. (2024) Fig. 6 — thick: current reproduction; "
        "dashed: published model; symbols: experiments",
        fontsize=13,
    )
    target = final_dir / args.figure_name
    fig.savefig(target, dpi=220)
    plt.close(fig)
    print(target, flush=True)

    reference = final_dir / "fig6_paper_reference.png"
    if reference.exists():
        left = plt.imread(reference)
        right = plt.imread(target)
        height = 8.0
        widths = (
        ...
        fig, axes = plt.subplots(
            1, 2,
            figsize=(height * sum(widths), height),
            gridspec_kw={"width_ratios": widths},
        )
        for axis, image in zip(axes, (left, right)):
            axis.imshow(image)
            axis.axis("off")
        fig.tight_layout()
        comparison = final_dir / "fig6_comparison.png"
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--runs-dir", type=Path, default=FIG_DIR / "runs_v22")
    result.add_argument(
        "--fallback-runs-dir",
        type=Path,
        help="secondary completed-run directory used only for missing cases",
    )
    result.add_argument(
        "--figure-name", default="fig6_reproduction.png",
        help="output PNG name under the Fig. 6 final directory",
    )
    result.add_argument(
    ...
    result.add_argument(
        ...
        default="degraded",
        help="Eq. (15) energy: paper psi=g(d)*psi0 or diagnostic psi0",
    )
    result.add_argument(
        "--alpha-0",
        type=float,
        default=10.5,
        help="fatigue degradation scale in MPa; calibrated exclusively to air",
    )
    result.add_argument(
        "--fatigue-degradation-exponent",
        type=float,
        default=2.0,
        help="exponent q in (alpha_0/(alpha_bar+alpha_0))^q; q=2 is Cui",
    )
    result.add_argument(
        "--fatigue-cycle-driving",
        choices=("maximum", "range"),
        default="maximum",
        help="accumulate the maximum energy or the positive loading-range "
        "increment alpha_max-alpha_min in each fatigue cycle",
    )
    result.add_argument(
        "--hydrogen-scaled-alpha-n",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="scale alpha_n by local f_H(C); disable for literal constant "
        "alpha_n in Eq. (15)",
    )
    ...
    result.add_argument(
        "--delta-p-override",
        type=float,
        help="diagnostic load-range override in N for the selected case(s)",
    )
    ...
    result.add_argument(
        "--fixed-cycle-jump",
        action="store_true",
        help="disable adaptive jump reduction/growth for convergence benchmarks",
    )
    result.add_argument(
        "--reset-adaptive-jump-on-resume",
        action="store_true",
        help="restart a resumed run from --cycle-jump instead of the checkpoint's "
        "last adaptive jump",
    )
    result.add_argument(
        "--block-error-control",
        action="store_true",
        help="control the entire coupled cycle block by full-step/two-half-step "
        "error estimation; accept the two-half-step state",
    )
    result.add_argument(
        "--block-crack-increment-rtol",
        type=float,
        default=0.005,
        help="relative part of the mixed step-doubling crack-increment tolerance",
    )
    result.add_argument(
        "--block-crack-increment-atol-mm",
        type=float,
        default=5.0e-5,
        help="absolute part of the mixed crack-increment tolerance in mm",
    )
    result.add_argument(
        "--block-phase-rtol",
        type=float,
        default=0.01,
        help="relative L2 tolerance for the final phase field",
    )
    result.add_argument(
        "--block-concentration-rtol",
        type=float,
        default=0.005,
        help="relative L2 tolerance for the final concentration",
    )
    result.add_argument(
        "--block-memory-rtol",
        type=float,
        default=0.005,
        help="relative L2 tolerance for the finite-rate hydrogen memory state",
    )
    ...
    result.add_argument(
        ...
        default="average",
        help="advance each cycle block with mean hydrostatic stress or "
        "sequential minimum/maximum half-cycle operators",
    )
    result.add_argument(
        "--diffusion-time-integrator",
        choices=("block", "subcycle"),
        default="block",
        help="integrate DeltaN/f in one implicit block step or bounded substeps",
    )
    result.add_argument(
        "--diffusion-max-step-s",
        type=float,
        default=3600.0,
        help="maximum physical diffusion substep when --diffusion-time-integrator "
        "is subcycle",
    )
    result.add_argument(
        "--hydrogen-memory-tau-s",
        type=float,
        default=0.0,
        help="bulk hydrogen-degradation relaxation time; zero recovers the "
        "instantaneous Cui law",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.cycle_jump <= 0 or args.min_cycle_jump <= 0:
        raise ValueError("cycle jumps must be positive")
    if (
        args.fatigue_n <= 0
        or args.alpha_0 <= 0
        or args.fatigue_degradation_exponent <= 0
    ):
        raise ValueError("fatigue n, alpha_0 and degradation exponent must be positive")
    if args.min_cycle_jump > args.cycle_jump:
        raise ValueError("--min-cycle-jump cannot exceed --cycle-jump")
    if (
        args.block_crack_increment_rtol <= 0
        or args.block_crack_increment_atol_mm <= 0
        or args.block_phase_rtol <= 0
        or args.block_concentration_rtol <= 0
        or args.block_memory_rtol <= 0
    ):
        raise ValueError("block error-control tolerances must be positive")
    if args.hydrogen_memory_tau_s < 0:
        raise ValueError("--hydrogen-memory-tau-s cannot be negative")
    if args.fixed_cycle_jump and args.min_cycle_jump != args.cycle_jump:
        raise ValueError(
            "--fixed-cycle-jump requires --min-cycle-jump equal to --cycle-jump"
        )
