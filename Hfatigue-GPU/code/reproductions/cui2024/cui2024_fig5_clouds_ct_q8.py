#!/usr/bin/env python3
"""Cui et al. (2024) Fig. 5 — self-contained run + plot script.

Reproduces the phase-field and hydrogen-concentration contour snapshots
(crack extensions of 2, 8 and 16 mm) of
...
Representative case: f = 1 Hz, pH2 = 106 MPa, R = 0.1, DeltaP = 6250 N.
The file is fully self-contained: Q8 kernels, the cuDSS direct-solver
backend, the coupled deformation/phase-field/diffusion cycle loop and
the contour plotting all live here.  Only the CT mesh (generated once,
stored under outputs/) is read from disk.  The solver is identical to
the Fig. 6 script (cui2024_fig6_paris_curves_ct_q8.py).

Usage:
    python cui2024_fig5_clouds_ct_q8.py --case all   # run, then plot
    python cui2024_fig5_clouds_ct_q8.py --plot-only  # plot from snapshots
"""
...
FIG_DIR = ROOT / "outputs" / "cui_2024_hydrogen_fatigue" / "fig5"
PAPER_MESH = FIG_DIR / "mesh" / "cui_ct_half_q8_paper.msh"
...
VERSION = "cui2024_fig5_selfcontained_v1"

# --- Fig. 5 representative case (paper Fig. 5 caption). ---
CASES: dict[str, dict] = {
    "p106_r01_f1": {"pressure": 106.0, "ratio": 0.1, "frequency": 1.0, "delta_p": 6250.0},
}
...
def run_case(name: str, args) -> None:
    ...
    print("=" * 78)
    print(f"Cui 2024 Fig. 6 case {name} — Q8 upper-half CT")
    print(
    ...
    while cycle < args.max_cycles:
        ...
        for stagger in range(1, args.max_stagger + 1):
            ...
            fatigue_q = fatigue_factor(alpha_bar_trial)
            trial, dr = solve_d(
    ...
    summary = {
        "paper": "Cui et al., IJHE 72 (2024) 315-325, Fig. 5",
        "version": VERSION,
        "doi": "10.1016/j.ijhydene.2024.05.264",
        "case": name,
...
# =========================================================================
# Plotting: phase-field and hydrogen concentration clouds at 2/8/16 mm
# =========================================================================
def _mirrored_triangulation(nodes: np.ndarray, elements: np.ndarray):
    """Corner-node triangulation of the upper half, mirrored to y<0."""
    ...
    nnode = len(nodes)
    mirrored_nodes = np.concatenate(
        (nodes, nodes * np.array((1.0, -1.0))), axis=0
    )
    mirrored_triangles = np.concatenate(
        (triangles, triangles[:, ::-1] + nnode), axis=0
    )
    return mtri.Triangulation(
        mirrored_nodes[:, 0], mirrored_nodes[:, 1], mirrored_triangles
    )
...
def plot_figures(args) -> None:
    ...
    snapshot_dir = args.runs_dir / case / "snapshots"
    snapshots = [
        (milestone, snapshot_dir / f"da_{milestone:g}mm.npz")
        for milestone in MILESTONES_MM
    ]
    available = [(m, p) for m, p in snapshots if p.exists()]
    if not available:
        print(f"no snapshots yet under {snapshot_dir}; skipping plot")
        return

    fig, axes = plt.subplots(
        2, len(available),
        figsize=(4.4 * len(available), 6.4),
        constrained_layout=True,
    )
    axes = np.atleast_2d(axes)
    if axes.shape[0] != 2:
        axes = axes.T
    loaded = [(milestone, np.load(path)) for milestone, path in available]
    concentration_max = max(
        float(saved["concentration_wppm"].max()) for _, saved in loaded
    )
    concentration_max = max(concentration_max, 1.0e-6)
    for column, (milestone, saved) in enumerate(loaded):
        nodes = saved["nodes"]
        elements = saved["elements"]
        triangulation = _mirrored_triangulation(nodes, elements)
        damage = np.concatenate((saved["phase_field"],) * 2)
        wppm = np.concatenate((saved["concentration_wppm"],) * 2)

        axis = axes[0, column]
        shading_d = axis.tripcolor(
            triangulation, damage, shading="gouraud",
            cmap="jet", vmin=0.0, vmax=1.0,
        )
        axis.set_title(f"$a = a_0 + {milestone:g}$ mm", fontsize=11)

        axis = axes[1, column]
        shading_c = axis.tripcolor(
            triangulation, wppm, shading="gouraud",
            cmap="jet",
            vmin=0.0, vmax=concentration_max,
        )
        for axis in (axes[0, column], axes[1, column]):
            axis.set_aspect("equal")
            axis.axis("off")
    fig.colorbar(
        shading_d, ax=axes[0, :], orientation="horizontal", shrink=0.6,
        label="Damage phase field $d$",
    )
    fig.colorbar(
        shading_c, ax=axes[1, :], orientation="horizontal", shrink=0.6,
        label="Hydrogen concentration $C$ (wppm)",
    )
    config = CASES[case]
    fig.suptitle(
        "Cui et al. (2024) Fig. 5 reproduction: "
        f"$f$ = {config['frequency']:g} Hz, "
        f"$p_{{\\mathrm{{H}}_2}}$ = {config['pressure']:g} MPa, "
        f"$R$ = {config['ratio']:g}",
        fontsize=13,
    )
    target = final_dir / "fig5_clouds.png"
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--target-extension-mm", type=float, default=16.5)
